...
namespace SimpLedger.Repository.Configurations.Validation.Account
{
    public class UserAccountValidation()
    {
        ...
        public static void NotNullEmailAndPassword(UserAccountLogin user) 
        {
        ...
        public static void ValidEmailAndPassword(string email, string password)
        {
        ...
        public static void ValidInformation(UserAccountCreation user)
        {
            if (string.IsNullOrWhiteSpace(user.Email))
            {
                throw new BadRequest("Email is required");
            ...
            if (Convert.ToInt32(user.UserType) < 1 || Convert.ToInt32(user.UserType) > 2)
            {
                throw new BadRequest("Account type is required");
            }
            if (Convert.ToInt32(user.UserType) == 1)
            {
                if (string.IsNullOrWhiteSpace(user.BusinessName))
                {
                    throw new BadRequest("Business name is required");
                }
                if (string.IsNullOrWhiteSpace(user.BusinessAddress))
                {
                    throw new BadRequest("Business address is required");
                }
                if (string.IsNullOrWhiteSpace(user.BusinessEmail))
                {
                    throw new BadRequest("Business email is required");
                }
            }
        }
    }
